# General helpers for check writing that are not explicitly checks
...
set_status <- function(status) {
  ...
  if (any(grepl("mdq_result", ls(envir = .GlobalEnv)))) {
    ...
    if (!class(local_result) == "list")
      stop("Name 'mdq_result' copied from global environment was not a list.")
...
#'
#' This function sets the status to SUCCESS and, optionally, sets an output message.
#'
...
#'
#' This function sets the status to FAILURE and, optionally, sets an output message.
#'
#' @param message (character) Optional. A message to set as an output.
#'
#' @return This function is invoked for its side-effect.
...
save_output <- function(x, type = "text") {
  ...
  if (any(grepl("mdq_result", ls(envir = .GlobalEnv)))) {
    ...
    if (!class(local_result) == "list") stop("Name 'mdq_result' copied from global environment was not a list.")
...
# Helper function to assess current status of check
